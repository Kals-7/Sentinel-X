...
def get_interfaces():
    try:
        ...
        for interface in interfaces:
            ...
            friendly_interfaces.append({
                "id": interface,
                "name": friendly_name,
                "description": get_interface_description(interface)
            })
        
        return jsonify({"interfaces": friendly_interfaces})
    except Exception as e:
        # Fallback with friendly names
...
def get_friendly_interface_name(interface_id):
    """Convert technical interface ID to user-friendly name"""
    interface_mapping = {
        # Loopback interfaces
        "\\Device\\NPF_Loopback": "Localhost",
        "lo": "Localhost",
        "Loopback": "Localhost",
    ...
    if interface_id in interface_mapping:
        return interface_mapping[interface_id]
    
    # Check for partial matches
    interface_lower = interface_id.lower()
    for pattern, name in interface_mapping.items():
        if pattern.lower() in interface_lower:
            return name
    
    # Try to extract info from GUID-based interfaces
    if "{" in interface_id and "}" in interface_id:
        # This is likely a Windows GUID interface
        return detect_windows_interface_type(interface_id)
    
    # Default fallback with some cleanup
    cleaned_name = interface_id.replace("\\Device\\NPF_", "").replace("\\", "_").replace("/", "_")
    return cleaned_name if cleaned_name else "Unknown Interface"

def detect_windows_interface_type(interface_id):
    """Try to detect Windows interface type from GUID"""
    try:
        import psutil
        import uuid
        
        # Try to match with psutil network interfaces
        for interface_name, interface_addresses in psutil.net_if_addrs().items():
            for address in interface_addresses:
                # Try to find matching interface by some heuristic
                if hasattr(address, 'address') and address.address:
                    # This is a simplified detection - in reality, you'd need more sophisticated mapping
                    if "ethernet" in interface_name.lower() or "local" in interface_name.lower():
                        if "local" in interface_name.lower():
                            return "Localhost"
                        else:
                            return "Ethernet"
                    elif "wifi" in interface_name.lower() or "wireless" in interface_name.lower():
                        return "WiFi"
                    elif "virtual" in interface_name.lower() or "vethernet" in interface_name.lower():
                        return "Virtual Network"
                    elif "vpn" in interface_name.lower() or "tap" in interface_name.lower():
...
def get_interface_description(interface_id):
    """Get a human-readable description for the interface"""
    descriptions = {
        "Localhost": "Local loopback interface for testing",
        "Ethernet": "Wired network connection",
        "Ethernet 2": "Secondary wired network connection",
        "WiFi": "Wireless network connection",
        "WiFi 2": "Secondary wireless connection",
        "Virtual Network": "Virtual network adapter",
        "VPN Connection": "Virtual private network connection",
        "Docker Network": "Docker container network",
        "Network Adapter": "Network interface adapter"
    }
    
    friendly_name = get_friendly_interface_name(interface_id)
    return descriptions.get(friendly_name, "Network interface for packet capture")
